  ...
  for (part in parties) {
    ...
    for (treat in treatments) {
      ...
      if (group == 1) {
        ...
        # Exclude subjects who gave same rating to each coalition
        dta_restr <- dta_restr[which(dta_restr$rat_coal_ALLIANCE!=dta_restr$rat_coal_MSD |
                                       dta_restr$rat_coal_ALLIANCE!=dta_restr$rat_coal_SAPMP |
                                       dta_restr$rat_coal_ALLIANCE!=dta_restr$rat_coal_SAPMPLC),]
      }
      ...
      formula <- as.formula(paste(ptv, paste("t", "age", "age_sqrt", "sex", "as.factor(edu)", "rat_M", "rat_SAP", "rat_V", 
...
# Footnote 14: Showing that 65.6% of the respondents who reported high ratings for the SAP-MP-L-C coalition also rated the M-C-L-KD coalition better than or equal to the Moderate Party

dta_high_SAPMPLC <- dta[which(dta$rat_coal_SAPMPLC >= dta$rat_coal_SAPMP & dta$rat_coal_SAPMPLC>=dta$rat_coal_ALLIANCE &
                                dta$rat_coal_SAPMPLC>=dta$rat_coal_MSD),]

sum(dta_high_SAPMPLC$rat_coal_ALLIANCE>=dta_high_SAPMPLC$rat_M, na.rm = T)/nrow(dta_high_SAPMPLC[which(!is.na(dta_high_SAPMPLC$rat_M) &
                                                                                 !is.na(dta_high_SAPMPLC$rat_coal_ALLIANCE)),])