...
e <- list()
i <- 0
for (part in parties[1:8]) {
  i <- i+1
  interval <- if(part=="M") c(1:2) else if(part=="SAP") c(3:4) else c(1:4)
  if (i%%2==1 | i == 2) {lab <- c("Treatment M-C-L-KD", "Treatment M-SD", "Treatment SAP-MP",  
                                  "Treatment SAP-MP-L-C")[interval]} else {lab <- c("", "", "", "")[interval]}
  e[[i]]<- qplot(x    = treatment,
    ...
    scale_x_discrete(limits=c("Coalition ALLIANCE", "Coalition M-SD", "Coalition SAP-MP", 
                              "Coalition SAP-MP-L-C")[interval], labels = lab) +
    geom_errorbar(aes(
...
# Plot Figure 5
grid.arrange(e[[1]], e[[2]], ncol=1, top=textGrob("", gp=gpar(fontsize=12, font = 2)), widths=c(3), heights=c(1, 1.15))
